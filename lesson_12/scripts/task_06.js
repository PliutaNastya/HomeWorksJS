...
// Задача 06. Дано масив імен. Застосовуючи відповідне сортування та бінарний пошук визначити, чи є у масиві ім’я довжиною 5 символів і під яким індексом.
...
function binarySearch(arr, searchElement, start, end) {
	if (start <= end) {
		...
		if (arr[middle].length < searchElement)
...
let arr = res.arr
let indexOfFiveSymbolsName = binarySearch(arr, 5, 0, arr.length - 1)
document.write(`
	<p>Ім'я, яке складається з 5 символів, ${indexOfFiveSymbolsName !== -1 ? `знаходиться під індексом ${indexOfFiveSymbolsName}` : 'не існує в цьому масиві'}</p>
	`)